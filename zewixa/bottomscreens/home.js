...
export default function HomePage({ route }) {
  ...
  useEffect(() => {
    axios
      .get(`${API_CONFIG.BASE_URL}/api/hostels`)
      .then(({ data }) => setHostels(data))
      .catch(console.error);

    axios
      .get(`${API_CONFIG.BASE_URL}/api/get-apartment-data`)
  ...
  const applyFilter = (h) => {
    ...
    if (filter.gender && filter.gender !== "All" && h.gender !== filter.gender)
      return false;
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.toggleContainer}>
        ...
        >
          <Text
            style={[styles.toggleText, showHostels && styles.activeToggleText]}
          >
            Hostels
          </Text>
        </TouchableOpacity>
      ...
      <ScrollView contentContainerStyle={styles.list}>
        <View style={styles.grid}>
          {displayedItems.filter(applyFilter).map((item) => {
            ...
            const price =
              item.rent?.singleSharing || item.rent?.["1BHK"] || "N/A";
