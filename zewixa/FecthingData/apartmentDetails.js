...
export default function ApartmentDetails({ route }) {
    ...
    const getPhoto = (key) => {
        ...
        return uri ? `https://zewixa-jz2h.onrender.com${uri.startsWith('/') ? '' : '/'}${uri}` : null;
    };
    ...
    return (
        <ScrollView contentContainerStyle={styles.container}>
            ...
            <Animatable.View animation="fadeInDown" duration={800} style={styles.profileCard}>
                <Image
                    source={{ uri: apartment.ownerData?.profileImage ? `https://zewixa-jz2h.onrender.com${apartment.ownerData.profileImage}` : 'https://via.placeholder.com/90' }}
                    style={styles.ownerImage}
                />
                <View style={styles.ownerInfo}>
                    <Text style={styles.ownerName}>{apartment.ownerData?.name || 'Owner Name'}</Text>
                    {apartment.ownerData?.mobile1 && (
                        <Text style={styles.ownerPhone}><Ionicons name="call-outline" size={16} /> {apartment.ownerData.mobile1}</Text>
                    )}
                    {apartment.ownerData?.mobile2 && (
                        <Text style={styles.ownerPhone}><Ionicons name="call-outline" size={16} /> {apartment.ownerData.mobile2}</Text>
                    )}
                    <Text style={styles.ownerLocation}><Ionicons name="location-outline" size={16} /> {apartment.location || 'Location not available'}</Text>
                    <Text style={styles.ownerEmail}><Ionicons name="mail-outline" size={16} /> {apartment.ownerData?.email || 'Email not available'}</Text>
                </View>
            </Animatable.View>

            {/* Amenities & Security */}
            <Animatable.View animation="fadeInUp" duration={900} style={styles.infoCard}>
                <Text style={styles.sectionTitle}> Amenities & Security</Text>
                {[
                    { label: 'WiFi', value: apartment.wifiAvailable === 'yes' ? 'Available' : 'Not Available', icon: <MaterialIcons name="wifi" size={20} color="#6846bd" /> },
                    { label: 'Electricity Included', value: apartment.isElectricityIncluded === 'yes' ? 'Yes' : 'No', icon: <Ionicons name="flash-outline" size={20} color="#6846bd" /> },
                    { label: 'CCTV', value: apartment.security?.cctv ? 'Yes' : 'No', icon: <Ionicons name="videocam" size={20} color="#6846bd" /> },
                    { label: 'Security Guards', value: apartment.security?.securityGuards ? 'Yes' : 'No', icon: <MaterialCommunityIcons name="security" size={20} color="#6846bd" /> },
                    { label: 'Gated Community', value: apartment.security?.gatedCommunity ? 'Yes' : 'No', icon: <Ionicons name="home" size={20} color="#6846bd" /> },
                    { label: 'Fire Safety', value: apartment.security?.fireSafety ? 'Yes' : 'No', icon: <MaterialCommunityIcons name="fire" size={20} color="#6846bd" /> },
                ].map((row, idx) => (
            ...
            <Animatable.View animation="fadeInUp" duration={900} style={styles.rentCard}>
                ...
                <View style={styles.rentTable}>
                    ...
                    <View style={[styles.rentRow, styles.rentHeader]}>
                        ...
                        <Text style={[styles.rentText, styles.rentHeaderText]}>Advance</Text>
                        <Text style={[styles.rentText, styles.rentHeaderText]}>Maintenance</Text>
                    </View>
                    {/* Table Rows */}
...
const styles = StyleSheet.create({
    ...
    profileCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 18, borderRadius: 16, marginBottom: 16, elevation: 3 },
    ...
    galleryGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', marginBottom: 16 },
    ...
    rentTable: {},
    rentRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#eee' },
    rentText: { fontSize: 14, color: '#333', fontWeight: '500', flex: 1, textAlign: 'center' },
    ...
    actionBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#6846bd', paddingVertical: 12, paddingHorizontal: 20, borderRadius: 30 },
