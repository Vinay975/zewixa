  ...
  const ApartmentData = () => {
    ...
    async function handleSubmit() {
      ...
      try {
        ...
        // Owner email
        data.append("ownerEmail", ownerData?.email || "");

        // Simple fields
        data.append("location", formData.location);
        data.append("wifiAvailable", formData.wifiAvailable);
        data.append("isElectricityIncluded", formData.isElectricityIncluded);

        // JSON fields
        data.append("security", JSON.stringify(formData.security));
        data.append("bhkUnits", JSON.stringify(formData.bhkUnits));

        // Photos (loop through all)
        for (const [key, value] of Object.entries(formData.photos)) {
          if (value) {
